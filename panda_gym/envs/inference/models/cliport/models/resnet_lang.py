...
from resnet import ConvBlock, IdentityBlock
...
class ResNet43_8s_lang(nn.Module):
    ...
    def _make_layers(self):
        ...
        self.text_encoder = DistilBertModel.from_pretrained('distilbert-base-uncased').to(self.device)
        self.text_fc = nn.Linear(768, 1024).to(self.device)
        ...
        self.lang_proj1 = nn.Linear(self.proj_input_dim, 512).to(self.device)
        self.lang_proj2 = nn.Linear(self.proj_input_dim, 256).to(self.device)
        self.lang_proj3 = nn.Linear(self.proj_input_dim, 128).to(self.device)
...
if __name__ == '__main__':
    cfg = {'train': {'batchnorm': False, 'lang_fusion_type': 'mult'}}
    network = ResNet43_8s_lang((240,240,3), 1, cfg, 'cuda:0', None)
